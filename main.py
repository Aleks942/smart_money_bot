    ...
    try:
        ...
        if side == "LONG":
            ...
            if tp1 is not None:
                ext2 = max(
                    h4_atr * 2.8,
                    abs(tp1 - float(entry_price)) if entry_price is not None else h4_atr * 2.0
                )
                tp2 = tp1 + ext2 * 0.5
        ...
        if status in ("SWING SETUP", "SWING TRIGGER") and not late:
            ...
            elif (not room_ok) and (not soft_room_pass):
...
if __name__ == "__main__":
    ...
    SCAN_BATCH = int(os.getenv("SCAN_BATCH") or "100")
        ...
        try:
            ...
            if alerts:
                ...
                top_alerts = [
                    ...
                    and (
                        (
                            float(s.get("score", 0)) >= 5.5
                            and int(s.get("acc_score", 0)) >= 2
                        )
                        or
                        (
                            float(s.get("score", 0)) >= 2
                            and int(s.get("acc_score", 0)) >= 2
                            and str(s.get("stage", "")) in ["ACCUMULATION", "TRANSITION"]
                        )
                    )
                ][:TOP_ALERTS_LIMIT]
